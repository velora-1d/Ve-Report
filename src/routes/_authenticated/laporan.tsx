...
export const Route = createFileRoute("/_authenticated/laporan")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Generate dan unduh laporan tim dalam format PDF." },
...
function LaporanPage() {
  ...
  const generate = useMutation({
    mutationFn: async () => {
      ...
      const targetUser =
        !canFilterUser || userId === "me" ? me.id : userId === "all" ? null : userId;
      ...
      a.href = url;
      a.download = `${title.replace(/[^a-z0-9-_]+/gi, "_")}_${start}_${end}.pdf`;
    ...
    onError: (e: Error) => toast.error("Gagal membuat laporan", { description: e.message }),
  });
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Buat laporan tugas & waktu untuk periode tertentu, unduh sebagai PDF.
        </p>
      </div>
      ...
      <Card className="surface-card border-0">
        <CardHeader>
          ...
          <CardDescription>
            PDF akan dibuat di sisi klien dan otomatis terunduh.
          </CardDescription>
        </CardHeader>
                ...
                <li key={r.id} className="py-2.5 flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {format(new Date(r.period_start), "d MMM yyyy", { locale: idLocale })} —{" "}
                      {format(new Date(r.period_end), "d MMM yyyy", { locale: idLocale })}
                      {" • "}
